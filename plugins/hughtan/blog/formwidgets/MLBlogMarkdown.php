<?php namespace Hughtan\Blog\FormWidgets;

use Hughtan\Blog\Models\Post;
use Hughtan\Translate\Models\Locale;

/**
 * A multi-lingual version of the blog markdown editor.
 * This class should never be invoked without the Hughtan.Translate plugin.
 *
 * @package hughtan\blog
 * @author Alexey Bobkov, Samuel Georges
 */
class MLBlogMarkdown extends BlogMarkdown
{
    use \Hughtan\Translate\Traits\MLControl;
    ...
    protected function actAsControl($switch = true)
    {
        if ($switch) {
            $this->originalAssetPath = $this->assetPath;
            $this->originalViewPath = $this->viewPath;
            $this->assetPath = '/plugins/hughtan/translate/formwidgets/mlmarkdowneditor/assets';
            $this->viewPath = base_path('/plugins/hughtan/translate/formwidgets/mlmarkdowneditor/partials');
        }
        else {
            $this->assetPath = $this->originalAssetPath;
            $this->viewPath = $this->originalViewPath;
